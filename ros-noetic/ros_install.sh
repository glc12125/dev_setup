...
SYSTEM_PACKAGE_TYPE=""
SYSTEM_PACKAGE_SET=""
ROS_PACKAGE_SET="ros-noetic-desktop-full"
...
#####################################################################################################
# Install dependencies for Noetic
#####################################################################################################
...
rosdep update
echo "$passwd" | sudo -S echo "source /opt/ros/noetic/setup.bash" >> ~/.bashrc
source ~/.bashrc
...
echo "$passwd" | sudo -S apt-get -y install ros-noetic-mavros \
...
cd $ROS_WORKSPACE
pwd
echo "$passwd" | sudo -S rosdep install --from-paths src --ignore-src --rosdistro noetic -y
print_green "Done !"
